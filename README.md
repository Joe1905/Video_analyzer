...
Configured topics are queried through SociaVault Top Search with the supported `publish_time`, `sort_by`, and `cursor` parameters. `HOT_VIDEO_TOPIC_SORT_BY` defaults to `most-liked`, and `HOT_VIDEO_TOPIC_MAX_PAGES` caps cursor pagination at `3` pages per topic.
SociaVault `videos-popular` and `trending` provide the broader discovery stream. `HOT_VIDEO_POPULAR_SORTS` controls the popular sort list (default: `views,likes`), and `HOT_VIDEO_POPULAR_MAX_PAGES` caps its dynamic pagination (default: `15`).
The report target count is the report analysis count setting (default: `10`), so displayed and analyzed videos stay aligned.
Ranking guarantees at most one leading candidate per configured topic, then fills all remaining slots by the shared hot score across topic and discovery-stream candidates.
