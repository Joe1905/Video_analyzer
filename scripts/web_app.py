...
import proxy_pool
import product_video_workspace
...
NAV_ITEMS = [
    ...
    {"key": "metrics", "href": "/metrics", "label": "视频列表", "title": "视频列表", "icon": '<path d="M4 19V5"/><path d="M20 19H4"/><path d="M8 16v-5"/><path d="M12 16V8"/><path d="M16 16v-7"/>'},
...
def fastmoss_report_prompt_instruction(route: dict[str, Any]) -> str:
    ...
    return (
        ...
        "事实边界优先于叙事完整性：渠道占比、广告归因或关联内容数量可以表述为观察到较高关联或归因占比，"
...
def fastmoss_report_style_instruction(route: dict[str, Any]) -> str:
    """Ask for a complete evidence-led report without dictating its prose structure."""
...
def handle_lan_chat_get(handler: BaseHTTPRequestHandler, parsed) -> bool:
    path = parsed.path
    try:
        preview_match = re.fullmatch(r"/api/lan-chat/drive/([0-9a-f]{32})/preview", path)
        if preview_match:
            ticket = parse_qs(parsed.query).get("ticket", [""])[0]
            source, name, content_type, size = lan_chat_store.drive_preview_info(preview_match.group(1), ticket)
            file_response(handler, source, content_type, name, size, download=False)
            return True
        if path == "/api/lan-chat/drive":
            json_response(handler, HTTPStatus.OK, {"files": lan_chat_store.drive_list(_lan_chat_token(handler))})
            return True
...
def handle_lan_chat_post(handler: BaseHTTPRequestHandler, parsed) -> bool:
    ...
    try:
        preview_match = re.fullmatch(r"/api/lan-chat/drive/([0-9a-f]{32})/preview", path)
        if preview_match:
            ticket = lan_chat_store.drive_preview(_lan_chat_token(handler), preview_match.group(1))
            json_response(handler, HTTPStatus.OK, {"url": f"{path}?ticket={ticket}"})
            return True
        if path == "/api/lan-chat/drive/upload":
            token = _lan_chat_token(handler)
            lan_chat_store.authenticate(token)
            try:
                length = int(handler.headers.get("Content-Length", "0") or "0")
                ...
                raise LanChatError("请求长度无效") from exc
            if length <= 0 or length > FILE_TRANSFER_MAX_BYTES + 2 * 1024 * 1024:
                raise LanChatError("上传内容为空或超过 10GB 限制", 413)
            if not handler.headers.get("Content-Type", "").lower().startswith("multipart/form-data"):
                raise LanChatError("请使用文件上传表单")
            form = cgi.FieldStorage(fp=handler.rfile, headers=handler.headers, environ={
                "REQUEST_METHOD": "POST", "CONTENT_TYPE": handler.headers.get("Content-Type", ""),
                "CONTENT_LENGTH": str(length),
            })
            item = form["file"] if "file" in form else None
            if item is None or isinstance(item, list) or not getattr(item, "file", None):
                raise LanChatError("请选择一个文件")
            result = lan_chat_store.drive_upload(token, item.filename, item.file)
            json_response(handler, HTTPStatus.CREATED, {"file": result})
            return True
        drive_match = re.fullmatch(r"/api/lan-chat/drive/([0-9a-f]{32})/(download|delete)", path)
        if drive_match:
            token = _lan_chat_token(handler)
            if drive_match.group(2) == "delete":
                _lan_chat_request_json(handler)
                lan_chat_store.drive_delete(token, drive_match.group(1))
                ...
                body = parse_qs(handler.rfile.read(length).decode("utf-8"))
                token = token or body.get("token", [""])[0]
                source, name, content_type, size = lan_chat_store.drive_info(token, drive_match.group(1))
                file_response(handler, source, content_type, name, size)
            return True
    ...
    def handle_proxy_api_post(self, path: str) -> None:
        try:
            if path == "/api/proxy/publish/jobs":
                ...
                drive_id = str(form.getfirst("drive_file_id") or "")
                if drive_id:
                    from types import SimpleNamespace
                    try:
                        source, name, content_type, size = lan_chat_store.drive_info(_lan_chat_token(self), drive_id)
                    except LanChatError as exc:
                        return json_response(self, exc.status, {"error": str(exc)})
                    if "video" in form:
                        raise ValueError("请仅选择一种视频来源")
                    if size > tiktok_studio_publish.MAX_UPLOAD_BYTES:
                        raise ValueError("视频超过 2GB 上传限制")
                    with source.open("rb") as stream:
                        form.list.append(SimpleNamespace(name="video", filename=name, type=content_type, file=stream))
                        result = tiktok_studio_publish.create_job(form)
                    return json_response(self, HTTPStatus.ACCEPTED, result)
...
UI_ASSET_VERSION = "20260921-publish-session"
METRICS_HTML = (SCRIPTS_DIR / "static" / "metrics.html").read_text(encoding="utf-8").replace("__UI_ASSET_VERSION__", UI_ASSET_VERSION)
