...
def handle_feishu_capability_post(handler: BaseHTTPRequestHandler, parsed) -> bool:
    if parsed.path not in {
        ...
        return False
    try:
        payload = _feishu_request_json(handler)
...
def _feishu_request_json(handler: BaseHTTPRequestHandler) -> dict[str, Any]:
    try:
        length = int(handler.headers.get("Content-Length", "0") or "0")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            route_match = WEB_ROUTER.resolve("POST", parsed.path)
        except (RouteNotFound, MethodNotAllowed):
            pass
        else:
            return route_match.handler(self, route_match.params)
        ...
        if handle_feishu_capability_post(self, parsed):
            return
        if parsed.path.startswith("/api/lan-chat/"):
            return lan_chat_post(self)
        if parsed.path.startswith("/amazon/"):
            return json_response(self, HTTPStatus.NOT_FOUND, {"error": "Not found"})
        if parsed.path.startswith("/chuhaijiang/"):
            return json_response(self, HTTPStatus.NOT_FOUND, {"error": "Not found"})
    ...
    def do_HEAD(self) -> None:
        parsed = urlparse(self.path)
        if parsed.path.startswith("/api/lan-chat/media/") and lan_chat_head(self):
            return
...
lan_chat_post, lan_chat_head, lan_chat_stream = register_lan_chat_api_routes(
    ...
    current_global_owner_id=current_global_owner_id,
    cookie_value=_cookie_value,
    media_cookie=LAN_CHAT_MEDIA_COOKIE,
    feishu_login_options=_feishu_login_options,
