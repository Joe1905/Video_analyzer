...
def run_chat_deepseek(store: ChatStore, session, assistant_msg, user_text: str, provider: str = "home", enabled_tool_ids: set[str] | None = None) -> None:
    ...
    for m in session.messages[-20:]:
        if m.id == assistant_msg.id or m.status == "error":
            continue
    ...
    if force_mcp_tools and not forced_provider_domain_tool_available(provider, tools):
        ...
        return
    capability_gaps = fastmoss_required_capability_gaps(routing_text, tools) if provider == "fastmoss" else []
    if capability_gaps:
        capability_labels = {
    ...
    if playbook_instruction:
        messages.append({"role": "system", "content": playbook_instruction})
    print(
    ...
    for _ in range(max_tool_rounds):
        try:
            ...
            if evidence_gaps:
                ...
                messages.append({"role": "system", "content": fastmoss_evidence_instruction(evidence_gaps)})
                continue
