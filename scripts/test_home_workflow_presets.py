...
from scripts.social_tool_router import SOCIAVAULT_OFFICIAL_TOOL_NAMES
...
class HomeWorkflowPresetTests(unittest.TestCase):
    ...
    def test_home_modal_has_populated_tabs_and_system_page_entries(self) -> None:
        ...
        self.assertIn('data-official-tab="tiktok"', modal["tabs"])
        self.assertIn('data-official-tab="video-social"', modal["tabs"])
        self.assertIn('data-official-tab="community-ad"', modal["tabs"])
        self.assertIn('data-official-tab="system"', modal["tabs"])
        self.assertIn('data-official-panel="tiktok"', modal["panels"])
        self.assertIn('data-official-panel="video-social"', modal["panels"])
        self.assertIn('data-official-panel="community-ad"', modal["panels"])
    ...
    def test_every_official_sociavault_tool_has_a_home_preset(self) -> None:
        registered = {
            tool_id.removeprefix("sociavault__")
            for preset in web_app.HOME_WORKFLOW_PRESETS.values()
            for tool_id in preset["tools"]
            if tool_id.startswith("sociavault__")
        }
        self.assertSetEqual(set(SOCIAVAULT_OFFICIAL_TOOL_NAMES), registered)
