...
class UIContractTest(unittest.TestCase):
    def test_sellersprite_renderer_uses_the_neutral_semantic_renderer(self) -> None:
    ...
    def test_runtime_uses_placeholders_and_external_assets(self) -> None:
        ...
        self.assertIn('"label": "\\u51fa\\u6d77\\u5320"', app)
        self.assertIn('"label": "账号运营台"', app)
        self.assertIn('<rect x="4.5" y="4.5" width="6.25"', app)
    ...
    def test_global_identity_uses_feishu_avatar_in_dock_and_compact_picker(self) -> None:
        ...
        self.assertIn("margin: auto auto 12px", css)
    ...
    def test_chat_history_controls_are_shared_across_providers(self) -> None:
        ...
        self.assertIn('class="mobile-session-toggle"', app)
        self.assertIn("M5.5 5.5h13", app)
        self.assertIn(".ui-header > .mobile-session-toggle", css)
        self.assertIn("order: 2", css)
        self.assertIn("background: transparent", css)
        self.assertIn('addEventListener("drop"', chat)
        ...
        self.assertNotIn("enabledToolMasks", chat)
        self.assertNotIn("TOOL_SELECTION", chat)
        self.assertNotIn("#toolModal", css)
        self.assertIn('class="tool-card-head"', chat)
        self.assertIn('class="tool-log-count"', chat)
        self.assertIn(".tool-card .tool-status.running", css)
        self.assertNotIn('id="headerToolBtn"', app)
