...
def rolling_provider_capabilities(provider: str, task: dict[str, Any], state: dict[str, Any]) -> set[str]:
    """Return the LLM-visible current/next-hop capability window.

    This is intentionally broader than the legacy staged graph: it removes fixed
    first-tool/phase decisions while keeping unrelated schemas out of the prompt.
    An empty result means the mapping is incomplete and callers must expose the
    provider's full enabled tool set.
    """
    objective = str(task.get("objective") or "lookup")
    scope = str(task.get("scope") or "keyword")
    entity_type = str(task.get("entity_type") or "none")
    ...
    if provider == "amazon":
        if entity_type == "asin" or state.get("has_asin"):
            capabilities = {"asin_detail", "asin_traffic", "asin_review"}
            if entity_type != "asin":
                capabilities.update({
                    "keyword_discovery", "market_discovery", "trend_validation",
                    "product_discovery", "category_resolution",
                })
            return capabilities
        if scope == "cross_category":
            capabilities = {
                "keyword_discovery", "market_discovery", "trend_validation",
                "product_discovery", "category_resolution",
            }
        elif entity_type in {"keyword", "category", "product"}:
            capabilities = {
                "keyword_discovery", "market_discovery", "trend_validation",
                "product_discovery", "category_resolution",
            }
    ...
    if provider == "fastmoss":
        if objective == "shop" or entity_type == "shop":
            return {"shop_discovery"} | (
                {"shop_detail", "shop_analysis"} if state.get("has_shop") else set()
            )
        if objective == "creator" or entity_type == "creator":
            return {"creator_discovery"} | (
                {"creator_detail", "creator_content"} if state.get("has_creator") else set()
            )
        if entity_type == "video" or objective == "content":
            capabilities = {"video_discovery", "product_discovery"}
            if state.get("has_video"):
                capabilities.add("video_detail")
            if state.get("has_product"):
                capabilities.add("product_content")
            return capabilities
        if entity_type in {"product", "product_id"}:
            return {"product_detail", "product_trend", "product_content"}
        if scope in {"cross_category", "category", "keyword"}:
            capabilities = {"category_discovery", "category_resolution", "product_discovery"}
            if state.get("has_category"):
                capabilities.add("category_context")
            if state.get("has_product"):
                capabilities.update({"product_detail", "product_trend", "product_content"})
            return capabilities
    return set()
...
def rolling_provider_tool_names(provider: str, task: dict[str, Any], state: dict[str, Any]) -> set[str]:
    capabilities = rolling_provider_capabilities(provider, task, state)
    if not capabilities:
        return set()
    mapping = (
        SELLERSPRITE_TOOL_CAPABILITIES if provider == "amazon"
        else FASTMOSS_TOOL_CAPABILITIES if provider == "fastmoss"
        else {}
    )
    return {
        name for name, capability in mapping.items()
