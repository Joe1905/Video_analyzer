...
CodeGraph 已在当前运行时代码 `2f9502f` 上重新同步。主要热点如下：
...
按以下互斥边界实施，每个运行时提交后都必须执行当时登记的完整门禁；Phase 1.1 当时登记门禁为 40 个自动化脚本，后续阶段当前登记门禁为 43 个：
...
按以下子阶段实施，每个子阶段独立提交并执行当前登记的 43 脚本门禁：
...
`proxy_pool.py` 不应与普通 web service 一起大搬。按事务边界拆，并在每个子阶段执行当前登记的 43 脚本门禁及代理专项故障注入：
...
先为现有 DeepSeek/Qwen 调用补 contract test，再引入 transport adapter。`hot_video_report.py` 保持显式 `max_tokens`，不能依赖隐式默认值。每个 transport/provider/chat 子阶段仍执行当前登记的 43 脚本门禁，专项测试不能替代完整门禁。
...
静态资源变化必须更新 `UI_ASSET_VERSION`。不得借重构恢复任何已退役 provider 页面或拆出三套聊天壳。每个可独立部署的资源拆分子阶段执行当前登记的 43 脚本门禁，并检查两个 Playwright 的桌面/移动 viewport、控制台和页面错误。
...
完成 Phase 1 整体验收后，下一批只能先做 Phase 2.1，再按顺序推进 Phase 2 的 health/page/static 骨架与 Phase 3 JobRegistry；不得跳过 Phase 3 直接迁移 Shop/Metrics/Amazon 业务路由。每个子阶段仍遵循 GitHub 同步、服务器 4004 构建、41 项确定性回归、部署、2 项 Playwright、日志扫描和 4002/4003 只读健康检查。
