...
CodeGraph 已在当前运行时代码 `e1b60af` 上重新同步。主要热点如下：

| 文件 | 规模 | 当前职责 | 判断 |
| --- | ---: | --- | --- |
| `scripts/web_app.py` | 13,205 行 / 608,547 字节（约 594.28 KiB） | 配置、页面装配、聊天 provider、四类临时任务、下载/店铺/指标/Amazon、遗留 GET/POST 路由、后台线程启动 | 第一重构对象，但必须分批拆；HTTP response/SSE helper、导入期配置、JSON 文件原语、health 与五个纯页面路由已完成抽取 |
...
按以下互斥边界实施，每个运行时提交后都必须执行当时登记的完整门禁；Phase 1.1 当时登记门禁为 40 个自动化脚本，后续阶段当前登记门禁为 48 个：
...
按以下子阶段实施，每个子阶段独立提交并执行当前登记的至少 48 脚本门禁；新增专项脚本时同步提高总数：
...
3. **2.2B 纯页面分批迁移（进行中）：** 2.2B-0 只读盘点、2.2B-1 `/report`/`/report/player`、2.2B-2 `/lan-chat`/`/tool` 与 2.2B-3 `/harness` 已完成；下一批先只读审计导入期缓存的 `/shop`、`/metrics`、`/taobao`。三页读取时机和缺失模板语义不同，必须冻结后再决定是否分别成批；`/harness-ca.crt` 继续留给 2.3 文件下载边界，`/amazon/`、`/chuhaijiang/` 两条显式 307 与其他严格尾斜杠行为必须分别锁定，不做全局 slash 归一化。每迁移一组都用路由表快照确认 URL、method 和 handler 唯一。
...
按以下子阶段实施，每个子阶段独立提交并执行当前登记的至少 48 脚本门禁：
...
`proxy_pool.py` 不应与普通 web service 一起大搬。按事务边界拆，并在每个子阶段执行当前登记的至少 48 脚本门禁及代理专项故障注入：
...
先为现有 DeepSeek/Qwen 调用补 contract test，再引入 transport adapter。`hot_video_report.py` 保持显式 `max_tokens`，不能依赖隐式默认值。每个 transport/provider/chat 子阶段仍执行当前登记的至少 48 脚本门禁，专项测试不能替代完整门禁。
...
静态资源变化必须更新 `UI_ASSET_VERSION`。不得借重构恢复任何已退役 provider 页面或拆出三套聊天壳。每个可独立部署的资源拆分子阶段执行当前登记的至少 48 脚本门禁，并检查两个 Playwright 的桌面/移动 viewport、控制台和页面错误。
...
Phase 0、0.5、1.1、2026-08-29 两个补漏阶段、Phase 1.2、Phase 1.3、Phase 2.1、Phase 2.2A、**Phase 2.2B-0、B-1、B-2 与 B-3** 已完成。下一步先实施 **Phase 2.2B-4 导入期缓存页面只读审计**；本步不改运行时代码，只冻结 `/shop`、`/metrics`、`/taobao` 的读取时机、缺失模板、导航注入和方法边界，再决定最小实施批次。

Phase 2.2B 按以下边界继续复用现有 Terra 子智能体，避免为同一长期任务无限新增执行记录：

1. 复用一个 Terra 逐页核对 `/shop`、`/metrics`、`/taobao` 的模块导入期读取表达式、文件缺失/解码失败表现、导航注入时机和 feature flag，禁止修改文件。
2. 复用另一个 Terra 交叉核对三页的 URL/query/尾斜杠、Content-Type/Cache-Control、HEAD/POST/DELETE 与相邻业务 API 边界，并给出可否同批的证据，禁止修改文件。
3. 主代理复核 CodeGraph 导入方向、旧分派 fallback、测试可冻结性和用户脏文件边界；只有同构语义有双重证据时才启动最小实现批次。实现后执行不少于当前 **46 项确定性回归 + 2 项 Playwright = 48 项** 的完整门禁，新增专项脚本时同步提高门禁总数。

Phase 2.2B-4 审计未完成前不迁移缓存页面；若三页语义不完全同构，则拆成独立代码子阶段，不为追求批量而引入通用模板 registry。`/harness-ca.crt`、静态资源和页面业务 API 仍不属于本批。继续按纯页面小批 → Phase 2.3 static → Phase 3 JobRegistry 的顺序推进；不得跳过 Phase 3 直接迁移 Shop/Metrics/Amazon 业务路由。
