...
当前范围更新（2026-09-08）：用户明确移除整个淘宝采集功能，实施记录见 §9.4；以下早期阶段记录属于历史快照，不再构成保留该功能的要求。现存页面清单移除此页后为 13 个，正式套件仍为 56 项。
...
| Phase 3.2 纯 JobRegistry | 已完成 | `fed5df9`；建立深复制注册、只读快照、日志追加和并发契约，服务器 55 项完整门禁全绿 |
| Phase 3.3.P1 原子字段更新能力 | 已完成 | `2509e0f`；增加字段校验、copy-on-write 原子更新与可选末日志，保持零运行时接线，服务器 55 项完整门禁全绿 |
...
Phase 3.3.P1 最终证据（2026-08-29）：`2509e0f` 只扩展 `scripts/jobs/registry.py` 与既有 `scripts/test_job_registry.py`。`update_fields` 接受字段 mapping，拒绝非 mapping、非字符串/私有/`log`/`updated_at`/未知字段和空操作；锁内以候选副本完成对象与字段值深复制、可选原文末日志和注入时钟，全部成功后才单次替换 Registry 条目，missing 为 `KeyError`。专项覆盖多字段原子提交、输入/快照嵌套隔离、raw final log、校验、stored/value deepcopy 与 clock 异常零部分写、锁释放，以及 Event 控制的 update/append 真实重叠和日志不丢失。交叉审查发现并收口既有 `append_log` 在时钟异常时先写日志的半状态；最终实现先取得时钟再触碰存储，同时不为逐条流日志深复制整个任务。没有生产调用者、领域字段表、状态机、callback、public replace/live get、I/O、线程职责、兼容层或反向依赖。

服务器只通过 4004 部署脚本构建，提交 `2509e0ffd359eed15eb69f7a993bd33dcd43e372`、镜像 `sha256:6e1804472f87ed31baa13d0e1c9aa9aed5bbc3b4444eb0baf199df91f1cca2f4`。Playwright 首次把测试容器自身的 loopback 误作 Web 地址而连接失败，随即切换为服务器 LAN 地址并从第 1 项完整重跑，最终通过 **53 项确定性回归 + 2 个 Playwright = 55 项**；13 页面、四类缺失 GET/SSE、未知 provider、三端健康、专属镜像、clean checkout、严重日志与镜像/环境/root 只读零残留均通过，4002/4003 镜像和启动时间未变化。启动时仍有 1 条既有静态代理节点同步 404 配置告警；它不依赖本批代码且擅自修复会触碰真实代理数据，因此记录为 Phase 5/运维边界的非阻断风险，不混入 P1。

**Phase 3.3.P1 总结与漂移审计：已完成。** 三路阶段审计均为 0 blocker。本批只建立零接线受锁更新能力，没有改变四套运行时字典/锁、worker、serializer、artifact 时序、URL、HTTP/SSE、数据或 UI；`jobs` 继续只依赖 stdlib。copy-on-write 对终态多字段更新是合理原子边界，但不用于高频逐条日志；未来接入大型 `result` 前必须先完成独立对象隔离决策。下一步 3.3.D0 只把 `public_download_job` 的 `result` 改为深复制并更新既有契约为“值相同、嵌套双向隔离”，不接入 Registry；D0 完整门禁通过后才进入 3.3.D1 Download 单域切换。

Phase 3.3.D0 最终证据（2026-08-31）：`c2fb2a1` 只将 `public_download_job` 的 `result` 改为 defensive deep copy，并在既有 `test_job_snapshot_contract.py` 中把 Python alias 基线更新为 JSON 值不变、顶层与嵌套对象双向隔离、`None` 原样保留；没有接入 Registry，也没有改变 Download worker、旧字典/锁、HTTP 状态/字段、SSE、80 条日志窗口或 POST queued/running 时序。完整门禁首次发现独立的日报续跑夹具把活动日期固定为 `2026-08-01`，到达默认 30 天保留边界后会在断点恢复前被真实清理；`2d8cb65` 只把该临时夹具的活动报告日期改为 `today_key()`，继续使用生产允许的默认保留策略并保留原 processed、重试、二次续跑与暂停断言，运行时代码零改动。修复后从第 1 项重新运行，提交 `2d8cb652b9613e3a9e0a516e99785b7b5f12e8ce`、专属镜像 `sha256:45a53b5620703758919113b9c40fcbe95c8a5cfa9ba4fbcea312632c40e3894f` 最终通过 **53 项确定性回归 + 2 个 Playwright = 55 项**；13 个活动页面、四类 missing GET/SSE、未知 provider、4002/4003/4004 健康、严重日志、服务器 clean checkout 及工作树/镜像/容器零残留均通过，4002/4003 未重启。

**Phase 3.3.D0 总结与漂移审计：已完成。** 三路 Terra 最终审计均为 0 blocker。D0 是独立可靠性修正，CodeGraph 与源码复核确认生产运行时仍只使用旧 Download 字典/锁，`JobRegistry` 没有生产导入或调用；没有双写、兼容层、反向依赖、通用抽象或跨域改动。服务器部署期间确认备用订阅已加载但 Mihomo `GLOBAL` 仍选择 default 节点，当前 GitHub HTTPS 与 Git 均已恢复；该真实代理选择属于 Proxy/运维边界，本阶段未擅自切换。服务器旧 `docker-compose 1.29.2` 与 Docker 29 的容器复用路径会触发 `ContainerConfig` 兼容错误，后续 4004 部署继续只使用 `bash scripts/deploy_ui_4004.sh` 的专属镜像与显式重建路径，不用裸 Compose 命令替代。下一步为 3.3.D1 Download 单域切换；只允许一次性迁移创建、失败输入登记、worker、GET、SSE 和 `build_video_feedback` 查询并删除旧字典/锁，禁止双写或保留兼容访问。

Phase 3.3.D1 最终证据（2026-08-31）：`2e1390d` 将 Download 的创建、无效输入 failed 登记、普通日志、worker 状态与终态、GET、专用 SSE 和 `build_video_feedback` 查询一次性切换到 `JobRegistry` 公共 API；旧 `download_jobs`/`download_jobs_lock` 与未被导入的一次性历史修复脚本已删除。worker 只从初始快照提取 `url/source`，普通日志走 `append_log`，成功与失败终态分别通过一次 `update_fields` 提交；失败路径在提交前重读最新快照选择有效日志。`public_download_job` 继续复用领域 adapter，保持 80 条日志窗口和 `result` 深复制；URL、HTTP 状态、JSON/SSE 字段、missing SSE 200、marker、POST 启动线程后 queued/running 竞态与 `output/download_jobs/` 数据目录均未改变。

专项在新镜像中覆盖 Registry、四域快照契约和真实 HTTP lifecycle，其中真实 `run_download_job` 的确定性成功/失败 fixture 验证终态原子更新、错误选择和嵌套深复制，无效 POST 使用固定 UUID 后再次 GET 验证 failed 登记。服务器只通过 4004 部署脚本构建，专属镜像为 `sha256:94a5ab20224d3f108e7dea2896295efc1e2c46d4f13ef2aeb3918144ea9f1dc2`；一次连续确定性运行通过 **53/53**，随后镜像内两个 Playwright 通过 **2/2**，合计 **55/55**。Playwright 首次误用未安装依赖的服务器宿主 Python，测试代码尚未启动即报模块缺失；改回包含浏览器依赖的新镜像后两项均通过，不属于产品或测试失败。部署黑盒的 13 页面、四类 missing GET/SSE、四个通用未知 provider 拒绝、三端健康、服务器 clean checkout、严重日志和本地/服务器/镜像/运行容器零残留均通过；4002/4003 启动时间未变化。

**Phase 3.3.D1 总结与漂移审计：已完成。** 两路 Terra 阶段后审计和主审均为 P0/P1/P2 0。Download 已形成单一 Registry 真源，不存在双写、兼容访问、私有成员访问、live stored object、反向依赖或跨域状态机抽象；`jobs` 继续只依赖 stdlib。删除废弃修复脚本不改变 Metrics 运行时。下一步为 3.3.D2 Metrics 单域切换；不得回改 Download，亦不得提前迁移 Shop 或 Amazon。

Phase 3.3.D2 最终证据（2026-08-31）：`280e739` 将 Metrics 的创建、普通日志、worker 状态与终态、GET、POST 和专用 SSE 一次性切换到 `JobRegistry` 公共 API；生产运行时中的旧 `metrics_jobs`/`metrics_jobs_lock` 已删除，测试继续保留旧名称的 AST 禁止断言。worker 从首次不可变快照提取 `target/endpoint`，普通日志走 `append_log`，终态走 `update_fields`；Registry 不读取 artifact、不执行命令。GET、POST 与 SSE 都按 `snapshot → 锁外 read_json → snapshot_metrics_job` 读取固定的 `output/tiktok_api/<id>/result.json`，保持 120 条日志窗口、200/404、missing SSE 200、marker、终态关闭、POST queued/running 竞态、endpoint/target 校验和结果 JSON 值。`7e4ab98` 只修正 lifecycle 在隔离测试根下的 Metrics 输出路径，`49f63e0` 只把 JSON store 的静态调用基线按净增两处从 47 更新为 49；两者均未改变生产行为。

阶段后测试审计发现两个 P2 并在进入下一阶段前收口：`f17e4fa` 用 fake `Popen` 直接执行真实 `run_metrics_command`，冻结命令首日志、stdout 归一和非零退出异常，并补齐 `music-popular` 空 target 的真实 HTTP 202/完成态；`edbaa38`、`607f839` 只修正 lifecycle 已替换 `subprocess` 的测试夹具，使局部测试恢复标准库常量并只 mock `Popen`。两次新专项失败均发生在 fixture 兼容性设置或预期断言之前，且没有启动真实外部进程；修正后先做服务器只读挂载预检，再只通过 4004 部署脚本构建最终提交 `607f839da3ca3205af417a44de087699e614ac26`，专属镜像为 `sha256:cd166f86506096119413f125a930c555874a35fc88452b9955eecdd675eea1d7`。最终从第 1 项连续通过 **53/53** 确定性回归和镜像内 **2/2** Playwright，合计 **55/55**；确定性发现规则保留离线 `test_api_cache.py`，明确排除两个手工外部 API 脚本，不用合成 key 把在线调用伪装成门禁。

部署黑盒的 13 个活动页面、四类 missing GET/SSE、四个通用未知 provider 拒绝、三端健康、服务器 clean checkout 和严重日志均通过；4002/4003 镜像及启动时间未变化。全字节审计额外发现本地与服务器的历史 API 缓存行和生成字节码，已只对命中缓存行做定向删除与 SQLite 安全压缩，并只删除命中的可再生 `.pyc`；未清空缓存库，未触碰报告、会话、账号、输出或其他真实数据。最终本地工作树、服务器 checkout、最终镜像和运行容器全字节零残留。
...
Phase 3.3.D3 最终证据（2026-08-31）：`a3e0a14` 只扩展四份既有测试，冻结 Shop 旧 store/Registry 私有访问为 0、真实 `run_shop_job` 成功/失败、真实命令日志与非零退出、POST queued/running/complete/failed、GET 200/404、missing SSE 200、120 条日志窗口、两份 artifact 的独立深复制与锁外读取顺序，以及 artifact 单独变化不触发四元 marker；同时把 R2 脱敏专项迁移到公开 Registry/job-id API，并把 `read_json` AST 基线按实测净增四处从 49 更新为 53。HTTP 使用非默认字段逐项验证 `source_type/region/max_pages/review_pages/analyze/related_videos`，自定义 prompt 在 POST、GET、SSE 和公开日志中均不可见。

`f6ab9b0` 只修改 `web_app.py`：建立唯一 `shop_job_registry`，删除 `shop_jobs/shop_jobs_lock`；worker 从首次不可变快照提取启动标量，日志和终态只使用 Registry 公共 API，失败状态、错误与末日志一次原子提交。GET、POST 与专用 Shop SSE 均按 `snapshot → shop_extract.json → shop_analysis.json → snapshot_shop_job` 在锁外读取固定目录；通用 SSE 与 Download、Metrics、Amazon 未修改，R2 真实 argv 与脱敏边界未回退。CodeGraph、AST 与 Terra 交叉终审确认无双写、兼容访问、live object、私有成员、Registry 锁内 I/O、共享业务基类或反向依赖。

服务器仅通过 4004 部署脚本构建提交 `f6ab9b0bbf375fea1f740a852e27f26d72e8298e`，专属镜像为 `sha256:f32680c14e08f2153235649f693840596ac1ee3370d1eb55a73eed6ca531e7e2`。新镜像从实际清单自动发现并连续通过 **54/54** 确定性回归和镜像内 **2/2** Playwright，合计 **56/56**；13 个活动页面、四类 missing GET/SSE、Shop 无效 POST、四个通用未知 provider、三端健康、服务器 clean checkout、严格代码异常日志和 root 只读挂载的 checkout/持久化数据/镜像/运行环境零残留均通过。4002/4003 镜像与启动时间未变化。启动日志仍有 1 条既有静态代理配置同步 404 告警，已在 Phase 5/运维边界登记，D3 未触碰该路径或真实代理数据。
...
7. **3.3.D0 Download result 隔离：** 独立可靠性提交只让 `public_download_job` 对 `result` 返回深复制，并把既有契约从 Python alias 改为值相同与嵌套双向隔离；不接入 Registry、不改 worker、字典/锁、HTTP JSON、SSE、日志窗口或 POST 时序。
8. **3.3.D1～D4 调用方切换：** D0 通过后按 Download → Metrics → Shop → Amazon 单域独立提交切换 registry；每切换一类都验证 API 查询与 SSE 字节契约。SSE 端点只消费快照，不直接访问可变任务对象；禁止双写旧/新 store。
9. **3.4 基类复核：** 只有发现除快照字段外还有稳定共享行为才评估基类；若只是少量字段复用，明确记录“不引入继承”。
...
7. **4.4B Upload 行为冻结（`ff46650`）：** 只扩充既有 `test_web_workflow_lifecycle.py`，以真实 Handler/multipart 冻结 Content-Length 0/超 2GiB、缺失/空 filename、单/多文件、部分成功/全部失败、原始错误文件名、来源优先级、文件名清洗、直接写盘、媒体删除、Registry/visibility/social 顺序、social 启动异常后的成功+错误并存，以及 UI_TEST 在解析和写入前拦截。原有 lifecycle 对真实 Registry 和后台 social 线程的污染风险同时改为构造期记录型 fake；没有新增测试或临时脚本。
8. **4.4B 结构迁移（`dada4f7`）：** 新增 `services/upload.py` 与 `routes/upload.py`；route 只负责 Content-Length、`cgi.FieldStorage`、`video` 提取、`source_tag/source` 归一和 HTTP 映射，service 只负责 `mkdir → safe_filename → 直接写盘 → media validate/delete → register → visibility → files append → social`。`web_app.py` 删除旧常量、手工 UI_TEST allowlist、内联 POST 分支和 `Handler.handle_upload`，仅保留七项显式注入与 Router 注册。生产代码净增 72 行，`web_app.py` 净减 40 行；无兼容层、双实现、通用上传框架或万能 VideoService。
9. **4.4B 测试接线与审计补强（`fe47e3f`、`78a1d39`、`fbbfd12`）：** 行为契约改为隔离的真实 UploadService + Router，不 patch service 私有依赖；Router AST 锁定唯一 POST、七项精确组合根注入、source/default 绑定和旧实现归零。首轮服务器门禁发现测试 fake 把 `Path` 而非文件句柄传给 `copyfileobj`，以独立测试提交修复；终审提出的 2GiB 精确值、source-only、空 source_tag 回退、可清理 filename、FieldStorage/service 双不可达哨兵全部收口后 P0/P1/P2 为 0。两份既有测试净增 311 行，属于高风险 multipart/写盘/外部边界契约，没有新增脚本。
...
16. **4.4D Translate 行为冻结（`1bbd001`）：** 只扩充既有 `test_web_workflow_lifecycle.py`，以真实 Handler/HTTP 和隔离记录型 runner 冻结输入 400、safe filename、`analysis_source/source` 优先级、content/direct/standard/direct-audit/direct-feedback 六种 artifact 映射、registry 风格输出目录、精确 argv/cwd/env/check/capture/text、缺输入、`CalledProcessError` 的 stderr→stdout→`str(exc)`→固定 fallback、其它异常传播、成功不校验目标输出和 UI_TEST 前置 409。没有真实子进程、网络、新测试脚本或临时脚本。
17. **4.4D 结构迁移（`ad6f459`）：** 新增 `services/translate.py` 与 `routes/translate.py`；route 负责 Content-Length、JSON/字段转换和 400/500/200 映射，service 负责校验、artifact 选择、输入存在性及同步 CLI 执行。`web_app.py` 删除旧 `Handler.handle_translate`、内联 POST 和手工 UI_TEST allowlist，仅显式注入 root、scripts dir、输出目录解析、safe filename、runner、environment 六项依赖并注册唯一路由。测试改为真实 TranslateService + Router 的公开注入，尾部 workflow 不再 patch 已绑定失效的全局 subprocess。`translate_analysis.py` 与日报直接复用保持原位；无兼容层、双实现、通用 subprocess runner 或万能 VideoService。
18. **4.4D 影响清单、部署与审计：** 服务器 `test_web_workflow_lifecycle.py`、`test_router.py`、`test_http_response_contract.py` 共 3/3 全绿，并通过相关 `py_compile`、`git diff --check`、CodeGraph 单向依赖、组合根和旧符号零残留；未运行聊天、代理、日报、Node、Playwright 或 56 项全量。仅执行 `bash scripts/deploy_ui_4004.sh`，4004 镜像/容器为 `sha256:960e2c531d0457c87dd19302d57275584b1e4484edd148d2ccb86d215584381f`；health、畸形 JSON、非法 source、非法 tab、缺输入黑盒分别为 200/400，严重日志 0，服务器 checkout clean。4002/4003 健康均为 200，镜像与启动时间不变。三路 Terra 终审与主审的行为漂移、架构边界、复用合理性和测试假绿均为 P0/P1/P2 全 0。下一步是 **4.4E Postprocess**，继续与 files/result/delete、Range/视频流分开迁移。
19. **4.4E Postprocess 行为冻结与结构迁移（`4558f0e`、`40e47a7`）：** 仅扩充既有 `test_web_workflow_lifecycle.py` 冻结 Content-Length/JSON、source 优先级、安全文件名、standard/direct artifact、prompt、report source/mode 文件、匹配 audit pair 清理、`analyze → report` 队列顺序、失败副作用、断连和 UI_TEST 前置 409；随后新增 `services/postprocess.py` 与 `routes/postprocess.py`，删除旧 `Handler.handle_postprocess`、内联 POST 和静态 allowlist，只在组合根显式注入输出目录解析、安全文件名和 queue enqueue。没有新增测试/临时脚本、兼容层、通用队列 facade 或万能 VideoService。
...
2026-09-08 用户明确要求：此问题先记录，不修复，主线继续重构。统一落档至 [系统缺陷与隐患记录](system-defects-and-risks.md) 的 SYS-001；不新增补偿状态、重试 API 或真实数据迁移，不再等待补偿方案决策。
...
### 9.4 Phase 5 中途功能退役（2026-09-08，已完成）

用户明确要求删除整个淘宝采集功能，不是改成直连或接入普通池。删除 collector、route、静态页、web 注册和定期清理入口、专用端口配置、独立 scope/reserve 入口及专属测试内容；保留通用代理数据库列与其他领域的 URL、鉴权、会话和任务行为。测试文件总数不变。

影响清单：代理 lifecycle、Router、缓存页面契约、web workflow lifecycle、web smoke、UI contract 和部署边界；共享代理连接的 TikTok 发布与 Instagram 采集专项一并运行。该批删除静态页但不重做其他页面，不启动已删除页面的 Playwright；完整 56 项仍留最终候选。

4004 只读数据预检发现：一个专用 scope 代理（id 11），四类其他领域表引用均为 0；两份专属档案、九条 stopped 会话和两条专属绑定。必须先停止 4004 web，避免旧 janitor 重建退役表；运行已登记 TTL 的备份清理脚本，确认只删除专属记录，再启动新代码，不能让旧 scope 在 init_db 中被重分配。共享 Mihomo 配置未发现专属 listener 标记，未修改该共享配置。专属目录先移入数据备份位置，不不可逆销毁历史证据。

本批完成后继续 runtime → repository 查询/事务 → accounts → publishing/collection → route 的 Phase 5 主线；SYS-001 仍仅落档。其他端启动时间变化只记录，后续仍只执行明确指定 4004 的运维命令，不以未知外部操作扩大停工范围。

收口证据：测试清理 `daa3fc5`，功能删除 `88ce158`（两者合计净减 1598 行）；临时工具 `831c344`、权限收紧 `3126924`、无关 FK 基线保持 `1e5c3da`，完成即由 `2bf926b` 删除，TTL 清空，没有新增正式测试脚本或转正脚本。隔离自检、真实 dry-run、备份清理均成功；备份在服务器 `data/taobao-retirement-backups/20260908T090816Z-b393ca5a511f4bf197f2f47b382d7c51/`，含 SQLite、三个专属目录和环境文件，目录 0700、数据库/环境备份 0600。清理后三表不存在、普通 scope 仍为 7 条、三个旧目录不再处于活动路径；17 条无关会话外键异常完全保持，另记 SYS-002，不在本批修复。
...
- 上述结构条件全部满足并完成阶段审计后，对最终候选一次性执行当前登记的完整自动化基线、13 个页面和全局隔离/日志检查；该次全量门禁通过才算重构完成。
...
7. **3.3.P1 原子字段更新纯能力（已完成，`2509e0f`）：** `JobRegistry` 已增加字段 mapping 的原子深复制更新和同锁可选末条日志，并收口 append 时钟异常半状态；零运行时接线，服务器 55 项完整门禁通过。
